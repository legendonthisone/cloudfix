...
2. a security group allowing public access on ports 80 and 443 is being created _(rule R2)_
   - evidence: `resource_changes[1].change.after.ingress[0].cidr_blocks[0]` = `0.0.0.0/0`  [no verification pass in this rung]
3. the load balancer replacement is a networking change that load balancer listeners depend on _(rule R6)_
   - evidence: `configuration.root_module.resources[0].expressions.load_balancer_arn.references[0]` = `aws_lb.public_api.arn`  [no verification pass in this rung]
