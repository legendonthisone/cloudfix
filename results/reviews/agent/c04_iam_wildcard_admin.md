...
1. the policy is being changed from scoped S3 operations to Action '*' on Resource '*', granting every action on every resource _(rule D2)_
   - evidence: `resource_changes[0].change.after.policy` = `{"Version":"2012-10-17","Statement":[{"Sid":"DeployAnything","Effect":"Allow","Action":"*","Resource":"*"}]}`  [verified against the plan]
2. this policy is attached to a role used by GitHub Actions in production _(rule D2)_
   - evidence: `resource_changes[1].change.after.role` = `github-actions-prod`  [verified against the plan]
